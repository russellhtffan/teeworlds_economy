...
void CGameContext::SendChat(int ChatterClientID, int Team, const char *pText)
{
	...
	Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, Team!=CHAT_ALL?"teamchat":"chat", aBuf);
        if (pText[0] == '/')
        {
            if (strncmp(pText, "/dummy", 5) == 0)
            {
                CreateNewDummy();
                return;
            }
            else if (strncmp(pText, "/buygun", 7) == 0)
            {
                CPlayer *pPlayer = m_apPlayers[ChatterClientID];
                CCharacter *cher = pPlayer->GetCharacter();
                if (pPlayer->m_Score > 4){
                    cher->GiveWeapon(WEAPON_GUN, 10);
                    pPlayer->m_Score -= 5;
                    return;
                }
                else{
                    SendChat(-1, CGameContext::CHAT_ALL, "You cant buy this item! This item cost 5 points");
                }
            }
            else if (strncmp(pText, "/buyshotgun", 11) == 0)
            {
                CPlayer *pPlayer = m_apPlayers[ChatterClientID];
                CCharacter *cher = pPlayer->GetCharacter();
                if (pPlayer->m_Score > 6){
                    cher->GiveWeapon(WEAPON_SHOTGUN, 10);
                    pPlayer->m_Score -= 7;
                    return;
                }
                else{
                    SendChat(-1, CGameContext::CHAT_ALL, "You cant buy this item! This item cost 7 points");
                }
            }
            else if (strncmp(pText, "/buygrenade", 11) == 0)
            {
                CPlayer *pPlayer = m_apPlayers[ChatterClientID];
                CCharacter *cher = pPlayer->GetCharacter();
                if (pPlayer->m_Score > 9){
                    cher->GiveWeapon(WEAPON_GRENADE, 10);
                    pPlayer->m_Score -= 10;
                    return;
                }
                else{
                    SendChat(-1, CGameContext::CHAT_ALL, "You cant buy this item! This item cost 10 points");
                }
            }
            else if (strncmp(pText, "/buykatana", 11) == 0)
            {
                CPlayer *pPlayer = m_apPlayers[ChatterClientID];
                CCharacter *cher = pPlayer->GetCharacter();
                if (pPlayer->m_Score > 19){
                    cher->GiveNinja();
                    pPlayer->m_Score -= 20;
                    return;
                }
                else{
                    SendChat(-1, CGameContext::CHAT_ALL, "You cant buy this item! This item cost 20 points");
                }
            }
            else if (strncmp(pText, "/help", 11) == 0)
            {
                CPlayer *pPlayer = m_apPlayers[ChatterClientID];
                SendChat(-1, CGameContext::CHAT_ALL, "To get commands list type /cmdlist");
            }
            else if (strncmp(pText, "/cmdlist", 11) == 0)
            {
                CPlayer *pPlayer = m_apPlayers[ChatterClientID];
                SendChat(-1, CGameContext::CHAT_ALL, "The commands: /buygun, /buyshotgun, /buygrenade, /buykatana, /help");
            }
            else
            {
                SendBroadcast("Unknown command", ChatterClientID);
            }
            return;
        }
